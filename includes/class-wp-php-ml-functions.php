...
class Php_ml_Functions
{
    ...
    function allow_access_private_post()
    {
        if (
            !empty($_GET)
            && !empty($_GET['key_php_ml'])
        ) {
            $key = base64_decode($_GET['key_php_ml']);
            $decode = $this->decode_key($_GET['key_php_ml']);
            if (!empty($decode['skip'])) {
                return;
            }
            unset($_GET['key_php_ml']);

            $key_db = md5(get_option(PHP_ML_APIKEY));
    ...
    public function get_link_post($custom_post)
    {
        ...
        if (strpos($link, '?') === false) {
            $link .= '?key_php_ml=' . $this->gen_key(false, $options);
        } else {
            $link .= '&key_php_ml=' . $this->gen_key(false, $options);
        }
        return $link;
    ...
    public static function uploadFile(
        ...
        try {
            if (!empty($api_key) && $api_key == get_option(PHP_ML_APIKEY)) {
                if (!empty($file)) {
                    if (empty($ext)) {
